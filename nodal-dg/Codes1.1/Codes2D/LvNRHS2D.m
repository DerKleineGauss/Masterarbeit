function [rhsu,Mu] = LvNRHS2D(u)

% function [rhsu] = LvNRHS2D(u)
% Purpose  : Evaluate RHS flux in 2D Liouville-von-Neumann equation using a stabilized internal penalty flux
...
fluxxu = nx.*du/2.0; qy = dudx - LIFT*(Fscale.*fluxxu);
fluxyu = ny.*du/2.0; qx = dudy - LIFT*(Fscale.*fluxyu);
