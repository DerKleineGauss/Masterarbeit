...
dentries = (1:Np*Np)';
dindices_row= zeros(Np*Np*K*(Nfaces+1),1);
dindices_col= zeros(Np*Np*K*(Nfaces+1),1);
dvalues= zeros(Np*Np*K*(Nfaces+1),1);

for k1=1:K
    if(~mod(k1,1000)) k1, end;
    rows1 = loc2glb(:,k1)*ones(1,Np);
    cols1 = rows1';
...
D = D(1:max(dentries)  -Np*Np,:);
D = myspconvert(D, Np*K, Np*K, 1e-15);
