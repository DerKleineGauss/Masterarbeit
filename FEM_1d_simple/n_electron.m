function [y] = n_electron(mu)
global hbar beta E_c m;
% trapez iteration
% error_ratio = 1e-10;
% % first iteration
% l = 1;
% a = 0;
% b = 1/E_c;
% T_l = (b-a)/2 * (0 + fermiTimesDensity(b, beta, mu, E_c));  %  für eps -> 0 bzw. E -> infty dominiert die exp-Funktion im Nenner, der Term wird 0
...
%%
y = real(integral( @(E)fermiTimesDensity(E,mu), E_c, (20/beta + mu))) * (2*m)^(3/2) / ( 2*pi^2 * hbar^3 );
end