...
\end{align}
Entwicklung in $\mathbb{V}_N$
\begin{align}
  \vb{x}\in K: \; u_N^K(\vb{x}) = \sum_{j=1}^{N_p} u_N^K(\vb{\xi}_j^K, t)\ell_j^K(\vb{x}) \\
  \vb{x}\in K: \; g_N^K(\vb{x}) = \sum_{j=1}^{N_p} g_N^K(\vb{\xi}_j^K, t)\ell_j^K(\vb{x})
\end{align}
\begin{align*}
...
Einsetzen :
\begin{align}
  iM^K\partial_t \vb{u}_N^K - (S^K_x)^T\vb{q}_{N,x}^K - (S^K_y)^T\vb{q}_{N,y}^K \\
  + \int_{\partial K} \sum_{j=1}^{N_p} \vb{q}^{*,K}(\vb{\xi}_j^K, t)\ell_j^K(\vb{x}) \ell_i^K(\vb{x}) \cdot \hat{\vb{n}}
  - M^K\vb{g}_N^K = 0
\end{align}
